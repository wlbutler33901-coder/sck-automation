...
**Adjustments** - TWO short sentences, 45 to 65 words total (v3.0 length discipline). Sentence one: the method framing plus the DOMINANT driver with its figure and the one fact explaining it (for sale timing that fact is the applied rate from engine metadata growth_pct_used - never assert a rate the engine did not use). Sentence two: names the NEXT-LARGEST drivers with their figures and NOTHING else. Signs always shown. CUT closing interpretive clauses ("keeping the in-submarket evidence largely intact" and similar) and CUT any parenthetical defining a category, since the five bullets below already define them. Pattern: "Each comparable is translated to the subject through five standardized adjustments applied additively to its unadjusted $/SF; the dominant driver is unit size at -X.XX%, size differences at 2% per 100 SF. Next largest are sale timing at +X.XX% and sale type at +X.XX%." Then these five bullets EXACTLY, category names bold, NO prose after the bullets:
