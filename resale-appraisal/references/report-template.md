...
- **Amenity Class**: social infrastructure only, 5% to 13% depending on class gap.
- **Finish Level**: 4.0% per common-area finish point of difference (Luxury 3, High-Quality 2, Basic 1, Utility 0; RAW, no tier floor). Capped at 12%.
- **Construction Materials**: 2.0% per materials point of difference (Tilt Wall 4, Block/Block+Tilt Wall 3, Metal/Metal+Block 2, Wood-Frame 1; default 3). Capped at 6%.
- **Sale Type** - 10.0% pre-construction incentive added to new-construction comps (the subject is a re-sale), per SCK's published 12 to 27% resale-over-new premium.
- **Unit Size**: 2% per 100 SF of size difference. Capped at 20%.

Table 3 - Comp Adjustments: # | Project | Unit # | Wealth Index | Sale Timing | Amenity Tier | Unit Size | Sale Type | Net Adj % | $/SF | Net Adj. $ PSF. LOCKED COLUMN SET, UNCHANGED BY v3.0: the two new components (Finish Level, Construction Materials) live in the component-averages table, the Adjustments narrative and the Net Adj % only. NEVER add a column here. LOCKED COLUMN SET: emit exactly these columns, in exactly this order, on every run, with NO conditional additions or omissions. Never add Submarket, Region, or any other context column to this table; submarket and region context belongs in the narrative and Table 5 only. The frontend parser matches headers exactly. Project and Unit # follow the SAME split and canonical-name rules as Table 2 (Address not repeated).
...
Table 4 - Adjustment Component Averages: | Component | Avg Adj % | for the five categories plus Total, directly below Table 3, bold label "**Table 4 - Adjustment Component Averages** (v3.0: SEVEN rows plus Total, in order Sale Timing, Wealth Index, Amenity Class, Unit Size, Sale Type, Finish Level, Construction Materials)". Total must tie to Table 1's Total Adjustment. This table feeds the application's Total Adjustment breakdown popup - always present.
