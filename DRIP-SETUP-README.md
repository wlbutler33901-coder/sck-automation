...
quarantined addresses from "04d - Email Verification". Drafts land in the
market@storagecondoking.com Outlook Drafts folder ready to review and send.
...
- Connectors: Supabase - Storage Condo King, Microsoft 365. Remove the rest.
...
Then run DRAFT mode. Pull the next 45 quarantined addresses from "04d - Email Verification" (skip if today's batch already exists), screen them against "04a - Email Suppression", personalize the plain text outreach per the skill, save each as an Outlook draft from market@storagecondoking.com, and stamp "Drafted At".
...
1. Morning: open Outlook Drafts, skim, press send on the batch.
