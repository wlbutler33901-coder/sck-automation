...
- Cadence: DAILY. Two passes each run: (a) REGION-WIDE skim of Tier 1 headlines (all five counties, last 48 hours), (b) DEEP DIVE on today's county per the permit scanner's rotation (references/sources.md tiers; match the county-of-the-day so permits and press for one county land the same morning). The deep dive follows Track A only until Tampa press sources are certified.
...
## 8. Logging and QA
Write run_started, per source results, and run_summary rows to Scan Activity Log with run_type swfl_news_scan, per ../swfl-permit-scanner/references/run-logging.md. Write each log row the moment its source finishes, never buffered to the end of the run.
