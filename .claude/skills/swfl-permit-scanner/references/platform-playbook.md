...
| Lee County DCD | SharePoint report app | Tier 1 static-file PDF (CERTIFIED, upgraded 2026-07-26) | BEST SOURCE: weekly reports at fixed, no-auth URLs under `/dcd/rpts/Documents/CurrentMonth/<PREFIX><TYPE>Week<1-5>.PDF` - a rolling 5-week window, no SharePoint auth needed, no monthly-report lag. Prefixes = sub-area (ULC=Unincorporated Lee County, VE=Village of Estero, NFM=North Fort Myers, BG=Boca Grande, CI=Captiva Island, LA=Lehigh Acres); TYPE=BPC is "Building Permits Issued - Commercial" (the one to read weekly; also BPR/BPD/BPT/BPP/BPRO/BPSO/BPEO/CPR exist but are lower-value). WeekN labels are NOT calendar-week numbers - they rotate; check each file's own "From:/To:" header and TimeLastModified to find the newest 2-3. Discovery: GET `https://www.leegov.com/dcd/rpts/_api/web/getfolderbyserverrelativeurl('/dcd/rpts/Documents/CurrentMonth')/files` (Accept: application/json;odata=verbose) lists all current filenames+mtimes; then plain GET the .PDF (no headers needed) and `pdftotext -layout` it (apt: poppler-utils). The old monthly `/dcd/rpts/Documents/<Area>/<Year>/<Mon>/<Area><Year><Mon>BPC.PDF` path (e.g. ULC2026JunBPC.PDF) still works as a Tier 3 fallback/backfill but lags ~1 week into the next month and is superseded by CurrentMonth for the daily scan. | 2026-07-26 | pypdf/cffi is broken in the CC cloud sandbox (missing `_cffi_backend` -> Rust panic) - use `pdftotext -layout`, not a Python PDF lib. |
...
| Village of Estero | Lee County DCD SharePoint (VE prefix) | Tier 1 static-file PDF (CERTIFIED, discovered 2026-07-26) | Estero permits ARE in the same Lee County DCD CurrentMonth feed under the VE prefix: `VEBPCWeek<1-5>.PDF` = "Building Permits Issued - Commercial - Village of Estero", same format/columns as the ULC report. Read alongside ULC each Sunday - no separate portal needed. The old estero-fl.gov static page / CGA portal remains a dead end (no date-searchable feed, broken TLS chain) and can be skipped now that this feed is known. | 2026-07-26 | 0 qualifying commercial permits issued in the 7/5-7/25/2026 window (all 3 weekly PDFs came back with an empty results table) - confirmed-empty, not a scan failure. |
...
UPDATE 2026-07-26 (Sunday cluster, Lee County unincorporated + Estero): discovered the `CurrentMonth` weekly-PDF feed (see Lee County DCD and Village of Estero rows above), which upgrades Estero from LIMITED to CERTIFIED and gives Lee County a no-lag weekly source instead of monthly-only. Read ULCBPCWeek1-3.PDF + VEBPCWeek1-3.PDF (7/5-7/25 window). Lee County: 7 new qualifying projects written (ids 72-78: Daniels Pkwy Apartments 289-unit/$56.3M, Gateway Marketplace Publix+Shops $8.04M, Nerano at Miromar Lakes, Boca Grande Health Clinic renovation, Crane Landing bldg, Wendy's - Ben Hill Griffin Pkwy, Cayo Costa State Park gift/maintenance shops) + 3 dedupe updates (progressions on existing ids 7, 8, 9). Village of Estero: 0 qualifying commercial permits in window (confirmed-empty via the VE feed). Sarasota Co / Naples / Punta Gorda / Palmetto blockers unchanged (out of today's cluster).
