...
- AMENITY VOCABULARY. "Key Amenities" holds ONLY canonical Amenity values from "Amenity Type Definition", comma separated, matched via the Amenity and Aliases columns. Facts that are not amenities (unit counts, acreage, pricing, deed structure, location context) go to scan_notes, never into Key Amenities. A genuinely new amenity type gets INSERTed into "Amenity Type Definition" with Status 'proposed', a one line Definition, and the observed phrasing in Aliases, then used in the row; never invent a synonym for something the table already covers.
