...
- AMENITY TIER. The scanner NEVER fills "Amenity Tier" at discovery; leave it null for enrichment's computed fill under the SCK Amenity Tier Standard v1.1 (see the enrichment skill). Record Social Programming only on concrete evidence of an actual program (named events, calendar, membership), never marketing language about community.
